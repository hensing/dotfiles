...
# zsh completion for cscli                                -*- shell-script -*-
...
_cscli()
{
    ...
    __cscli_debug "CURRENT: ${CURRENT}, words[*]: ${words[*]}"
    ...
    __cscli_debug "lastParam: ${lastParam}, lastChar: ${lastChar}"

    # For zsh, when completing a flag with an = (e.g., cscli -n=<TAB>)
    ...
    __cscli_debug "About to call: eval ${requestComp}"

    # Use eval to handle any environment variables and such
    out=$(eval ${requestComp} 2>/dev/null)
    __cscli_debug "completion output: ${out}"
    ...
    __cscli_debug "last line: ${lastLine}"
    ...
    if [ $((directive & shellCompDirectiveError)) -ne 0 ]; then
        __cscli_debug "Completion received error. Ignoring completions."
        return
    fi
    ...
    if [ $((directive & shellCompDirectiveFilterFileExt)) -ne 0 ]; then
        ...
        __cscli_debug "Calling _describe"
        if eval _describe $keepOrder "completions" completions $flagPrefix $noSpace; then
            __cscli_debug "_describe found some completions"

            # Return the success of having called _describe
            return 0
        else
            __cscli_debug "_describe did not find completions."
            __cscli_debug "Checking if we should do file completion."
            if [ $((directive & shellCompDirectiveNoFileComp)) -ne 0 ]; then
                __cscli_debug "deactivating file completion"
                ...
                __cscli_debug "Activating file completion"
